...
However, transportation systems are not exactly the same with thermodynamic systems. For one thing, the second law of thermodynamics doesn't exist in transportation context. For another, the significations of order in two systems are opposite. Hence, the important notion of entropy can not be introduced directly. To this end, this paper defined the transportation entropy by using the formula of thermodynamic ectropy that measures the order in thermodynamic context. Such transportation entropy is proportional to the traffic densities and hence it is an appropriate measurement of system disorder.
...
Furthermore, another difference between two systems is that the transportation network is always open. But, this paper has also shown that there exists the nominal situation in transportation systems, that is similar with the closed status in thermodynamic ones. We define the disturbances as the difference between real input flows and nominal ones. Such disturbances lead to a more strict version of dissipativity.

Finally, we would like to point out that, by bringing certain thermodynamic concepts and principles into transportation context, this paper provides a completely new view to transportation network. The results are applicable to analysis of system performances and design of traffic signal control strategies.
