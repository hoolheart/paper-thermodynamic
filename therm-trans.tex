% This paper tries to present the thermodynamic point of view for
% studying the transportation system. To show the university of this
% contribution, a general transportation model is used instead of the
% previous linear one.

% This new point of view is to analogize the transportation system to
% the thermodynamic system. The fundamental pair of correspondent
% concepts are the vehicle and the energy. The principles are
% introduced as well.

% The new attempt to make this a separate paper is to present the point
% of view more clearly. In particular, the relationship between Haddad
% entropy and Boltzmann entropy should be explored properly. The
% ectropy for transportation system is also needed to present to
% complete the approach.

% The approach of Shannon's entropy is also explored to present the
% notion of transportation information, which measures the system
...
streets.
%Therefore, the transportation system is considered as the network of
%traffic lanes in this paper.
...
%Furthermore, since for every lane, it is impossible to say negative
%queue length, which means $x_i\ge 0$, $\forall i\in\{1,\cdots,n\}$.
%Hence, we have
%\begin{equation}\label{equ:positive}
%U(k) \ge 0,\quad \forall k\in\mathbb{N}
%\end{equation}
%Now, by combining \eqref{equ:conservation_ex} and
%\eqref{equ:positive}, it is implied that
...
changes.
% For example, the heat can be transferred from hotter objects
% to colder ones, but it can never go in the opposite direction.
% Certain phenomenons have been studied and generated the second law
of
% thermodynamics \citep{cengel_thermodynamics:_2001}, which introduced
% a special concept.
...
%   \centering
%   % Requires \usepackage{graphicx}
%   \includegraphics{pics/carnot}\\
%   \caption{Carnot cycle}\label{fig:carnot}
% \end{figure}
