...
better organized.
In addition, we show that the thermal equilibrium arising in isolated thermodynamic system can be also introduced to correspond to
the state when all lanes share the same occupancy.  Contrary  to the second law of thermodynamic, this equilibrium occupancy leads  to a minimum entropy rather than to a maximum.
...
Contrary  to the second law of thermodynamic, this equilibrium
occupancy leads  to a minimum entropy rather than to a maximum.
...
% to be non-negative
on the non-negative orthant. Moreover, it is obvious
that all $\sigma_{i,j}=0$ and $d_{j}=0$ if and only if $x_j=0$,
$j\in\{1,\cdots,n\}$. This implies that if the $j$th lane
...
Indeed, the bigger number of vehicles increases the entropy and hence
increases $I(k)$, which implies the smaller speed of vehicles and
consequently the greater disorder.
...
\citep{Jacquez1993}.
On the other hand, the exchanges $\sigma_{i,j}$ are determined by the
...
$\lambda_{i,j}\in[0,1]$ is the proportion of $\sigma_{i,j}$ to such
leaving flow. This infers that
\begin{equation}
\label{eql}
\begin{split}
l_i(k)&=\sum_{j=1,j\neq i}^{n} \sigma_{i,j}(k)
...
$\vec{B}_i(x)=[B_{i,1}(x),\cdots,B_{i,n}(x)]^T$ in the following
manner
\begin{equation*}
...
The solution is given by the following LMI by applying Schur
...
In particular, if we chose $\mat{H}=h\mat{I}$, $h\in \mathbb{R}$, then
for any vector $\vec{\theta}$, $g(\vec{\theta})$ is non-positive if
and only if $|h+1|\leq 1$. Indeed, with $\mat{H}=h\mat{I}$ it follows
