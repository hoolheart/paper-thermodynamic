...
\begin{keyword}
Transportation systems\sep thermodynamic systems \sep entropy\sep dissipativity theory\sep nominal situation\sep equilibrium.
\end{keyword}
...
To solve it, various approaches have been applied to understand the phenomenons in transportation systems. Among them, the traffic flow theory has been the most widely used \citep{nathan_h_gartner_revised_2005}. This approach considers the vehicle movements as liquid flows, which matches the general impression of the traffic in modern cities. The traffic flow theory can effectively describe the macroscopic traffic phenomenons and has led to several well-known traffic signal control strategies, like TRANSYT \citep{robertson_tansyt_1969,hale_traffic_2005}, SCOOT \citep{bretherton_r_d_scoot_1982}, TUC \citep{diakaki_multivariable_2002}, etc. In this paper, we also apply some concepts in traffic flow theory to model the system. On the other hand, the kinematic wave theory has been very popular to study the speed-density relationship from microscopic point of view \citep{zhang_kinematic_2002,jin_multicommodity_2004}. Its first-order approximation led to the Cell Transmission Model (CTM) \citep{daganzo_cell_1995}, which has shown high accuracy in the simulation \citep{almasri_online_2005}. Hence, CTM has been used in traffic estimation \citep{CanudasdeWit2012,tampere_extended_2007} and traffic signal control \citep{Pohlmann2010}.
...
In addition, the applications of Petri nets (PNs) in modeling and control of transportation systems have been also conducted for over a decade \citep{ng_review_2013}. PN traffic models have been useful for analyzing performances and assisting intelligent traffic control.
...
In detail, this paper will firstly show the connections between the transportation system and the thermodynamic system so that the basic concepts can be introduced into the transportation context. Then, it will be demonstrated that the transportation system can have a similar notion of entropy to measure the system disorder. By considering this new entropy notion as the storage function, a dissipativity law will be presented to reduce the disorder and hence render the system better organized. Furthermore, it will be shown that the nominal situation in transportation systems is similar with the closed status in thermodynamic systems, and the equilibrium can be also introduced to represent the state when all traffic lanes have the same occupancy.
% the Shannon's entropy will be also applied to measure the information of the transportation system. Different with the transportation entropy, the information concept measures the transportation functionality and hence can be a good means to evaluate the system itself.
% The work in this paper can provide a novel thermodynamic point of view to explore the nature of the transportation system.
...
This paper has presented a novel thermodynamic point of view for the transportation system. Indeed, the comparison between thermodynamic systems and transportation systems has showed that they are very similar by regarding the vehicles as the energy and regarding the traffic lanes as the thermodynamic subsystems. Hence, this paper was able to introduce the thermodynamic concepts into the transportation context. In particular, it has been demonstrated that the transportation system can have a similar notion of the entropy to measure the system disorder. By considering the transportation entropy as the storage function, this paper has presented a dissipativity phenomenon for the transportation system to reduce its disorder and hence render the system better organized. Though this phenomenon doesn't exist naturally, it can be used as the objective for traffic signal control. Furthermore, it has been shown that the nominal situation in transportation systems is similar with the closed status in thermodynamic systems, and the equilibrium has been also introduced into the transportation context to represent the state when all traffic lanes have the same occupancy.
% Beside the classical thermodynamic approach, this paper also applied the Shannon's point of view to present the information of transportation system, which can measure the transportation functionality and hence can be a good means to evaluate the system.
The work in this paper is a completely new attempt to discover the nature of transportation systems, which can be further applied to analyze the system performance and design traffic signal control.
